...
- **The fourth tier is the one that matters.** Tiers 1–3 all ultimately need USDA to answer *this deployment* at some point; if it never does, they are empty together. `.github/workflows/refresh-feeds.yml` runs `scripts/refresh-feeds.mjs` on a GitHub runner four times a day — a different network with a different IP reputation — and commits the result, so the build ships with the data as a static asset needing no Blob store, no environment variable, and no cooperation from USDA at request time.
- **Why the 403 happens is not established, and the code says so.** Two commits in this repo assert opposite causes — `852c7d0` ("rejects clients without a browser user agent", and its title claims it fixed the 403) and `4c0676c` ("an IP and TLS fingerprint decision, not a header one", which removed those headers) — neither with a measurement, and no public bug report corroborates either. `4c0676c` also deleted the header matrix, which was the only instrument that could settle it. It is restored in `DIAG_HEADER_SETS`, reachable only from `/api/diag?probe=feeds`, where four variants (honest UA, browser UA, full browser headers, and a no-UA control) run in parallel and the endpoint states its own conclusion: which theory the result supports, or that the pattern fits rate limiting instead. The request path still sends one honest header set; nothing is adopted automatically, because presenting a browser handshake to a government API is a decision for a person.
